...
// Groups Preview tab (PLAN.MD §9.2): the 12 group tables computed from the
// player's own predicted scores, with qualification highlighting and the
// correctness overlay once the real group stage is final.
...
export function GroupsView({ payload }: { payload: GroupsPayload }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="font-display text-2xl font-bold text-slate-100">Your groups</h1>
        ...
        {payload.groups.map((g) => (
          <div key={g.name} className="glass rounded-2xl p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              {!g.complete ? (
                <Chip className="bg-white/5 text-slate-500">{g.predsMade}/6 predicted</Chip>
              ) : payload.qualifiedThirdGroups?.includes(g.name) ? (
